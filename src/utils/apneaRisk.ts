...
export type ApneaRiskLevel =
  "SIN_DATOS" | "NORMAL" | "BAJO" | "MODERADO" | "ALTO" | "CRITICO";
...
export function riskFromApneaEvents(
  apneaCount: number | null | undefined,
): RiskVisual {
  ...
  if (count === 0) {
    return {
      ...
      nextStep:
        "Por ahora no necesitas consulta. Sigue monitoreando con frecuencia.",
    };
  }
  if (count <= 2) {
    return {
      ...
      nextStep:
        "Mantén el monitoreo: completa también un registro de horas de sueño.",
    };
  }
  if (count <= 5) {
    return {
      ...
      nextStep:
        "Prueba dormir de lado y evita alcohol antes de acostarte. Vuelve a medir mañana.",
    };
  }
  if (count <= 9) {
    return {
      ...
      nextStep:
        "Consulta a un especialista del sueño y comparte esta información con tu médico.",
    };
  }
  return {
    level: "CRITICO",
    ...
    nextStep:
      "Busca atención médica lo antes posible. Activa las alertas de emergencia si aún no lo hiciste.",
  };
}

export function riskFromPredictionNivel(
  ...
  if (value === "CRITICO") {
    return {
      ...
      nextStep:
        "Protocolo de emergencia activado. Revisa el estado de la persona y a sus contactos.",
    };
  }
  if (value === "ALERTA") {
    return {
      ...
      softColor: palette.dangerSoft,
      interpretation:
        "El modelo marcó un evento de apnea probable en el fragmento.",
      nextStep:
        "Mantén la posición de lado y verifica que el monitoreo siga activo.",
    };
  }
  if (value === "NORMAL") {
    return {
  ...
  return {
    ...
    interpretation:
      "El modelo aún no devuelve un resultado para este fragmento.",
