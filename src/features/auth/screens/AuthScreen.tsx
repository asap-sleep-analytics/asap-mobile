...
import { getApiErrorMessage, loginUser, registerUser, socialLoginUser } from '../../../services/api';
...
type AuthStackParamList = { AuthScreen: undefined; Terms: undefined };
...
export default function AuthScreen() {
	...
	const legalPending =
		isRegisterMode && (!aceptaTerminosCondiciones || !aceptaConsentimientoDatos || !aceptaDisclaimerMedico);
	...
	const handleSocialLogin = async (provider: 'google' | 'apple') => {
		...
		try {
			...
			if (provider === 'google') {
				if (!googleRequest) {
					setError('Faltan los clientes de Google. Configura EXPO_PUBLIC_GOOGLE_* en tu .env.');
					return;
				}
				const result = await googlePromptAsync();
				if (result?.type !== 'success') {
					if (result?.type === 'error' && result.error) {
						setError(result.error.message || 'Google no completó el inicio de sesión.');
					}
					return;
				}
			...
			const response = await socialLoginUser({
				provider,
				id_token: idToken,
				nombre_completo: isRegisterMode && nombreCompleto.trim() ? nombreCompleto.trim() : undefined,
				ronca_habitualmente: roncaHabitualmente,
				cansancio_diurno: cansancioDiurno,
				...socialLegalPayload,
			});
			...
			const message = getApiErrorMessage(err, 'No fue posible iniciar sesión con el proveedor.');
				...
				<GlassCard>
					...
					{isRegisterMode ? (
						<>
							...
							<View style={styles.switchRow}>
								<Pressable style={styles.legalSwitchLabel} onPress={() => navigation.navigate('Terms')}>
									<Text style={styles.switchLabel}>
										Acepto los <Text style={styles.legalLink}>términos y condiciones</Text>
									</Text>
								...
								<Switch
									...
									trackColor={{ false: palette.borderSoft, true: palette.primarySoft }}
									thumbColor={aceptaTerminosCondiciones ? palette.primary : '#f4f4f5'}
								/>
							</View>
					...
					{legalPending ? (
						<Text style={styles.helperText}>Acepta los tres consentimientos para poder registrar.</Text>
					) : null}
						...
						{isRegisterMode ? (
							<Pressable onPress={() => switchMode('login')}>
								<Text style={styles.switchLink}>
									¿Ya tienes cuenta? <Text style={styles.switchLinkStrong}>Inicia sesión</Text>
								</Text>
							</Pressable>
						) : (
							<Pressable onPress={() => switchMode('register')}>
								<Text style={styles.switchLink}>
									¿No tienes cuenta? <Text style={styles.switchLinkStrong}>Regístrate</Text>
								</Text>
							</Pressable>
						)}
					</View>
