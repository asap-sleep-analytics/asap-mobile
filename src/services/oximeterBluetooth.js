...
export async function startOximeterReading(onReading) {
  ...
  if (pulseOx) {
    ...
    for (const char of chars) {
      ...
      if (
        isSpo2Characteristic(char.uuid) ||
        isPulseRateCharacteristic(char.uuid)
      ) {
  ...
  if (readingsSubscriptions.length === 0) {
    throw new Error(
      "El oxímetro no expone datos de SpO2. Revisa que esté encendido y en el dedo.",
    );
  }
}
...
async function getManager() {
  ...
  if (Platform.OS === "web") {
    throw new Error(
      "Bluetooth no está disponible en la versión web. Usa Android o iPhone con una compilación nativa.",
    );
  }

  if (isExpoGoEnvironment()) {
    throw new Error(
      "Bluetooth BLE no funciona en Expo Go. Necesitas una Development Build para conectar el oxímetro.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "Bluetooth no disponible en este build. Usa Development Build para conectar oxímetro.",
    );
  }

  const BleManagerClass =
    moduleRef?.BleManager ||
    moduleRef?.default?.BleManager ||
    moduleRef?.default;
  if (!BleManagerClass) {
    throw new Error(
      "No fue posible inicializar Bluetooth en este dispositivo.",
    );
  }
  ...
  try {
    ...
    if (
      String(error?.message || "")
        .toLowerCase()
        .includes("createclient")
    ) {
      throw new Error(
        "El módulo nativo de Bluetooth no está cargado. Usa una Development Build, no Expo Go.",
      );
    }
...
export async function scanOximeters(timeoutMs = 6000) {
  ...
  if (isExpoGoEnvironment()) {
    throw new Error(
      "Bluetooth BLE no funciona en Expo Go. Usa una Development Build para conectar el oxímetro.",
    );
  }
    ...
    bleManager.startDeviceScan(
      ...
      (error, device) => {
        if (error) {
          ...
          reject(
            new Error(
              error.message || "No se pudo escanear dispositivos Bluetooth.",
            ),
          );
          return;
        }
