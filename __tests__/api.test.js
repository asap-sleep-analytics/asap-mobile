...
describe('api error messages', () => {
  ...
  it('usa el fallback si no hay nada útil', () => {
    expect(getApiErrorMessage({})).toBe('No fue posible completar la solicitud.');
...
describe('endpoints de sleep', () => {
  ...
  it('startSleepSession hace POST a /api/v1/sleep/sesiones/iniciar', async () => {
    ...
    expect(mockedPost).toHaveBeenCalledWith('/api/v1/sleep/sesiones/iniciar', { modo: 'cell_only' });
  ...
  it('finishSleepSession usa el sessionId en la URL', async () => {
    ...
    expect(mockedPost).toHaveBeenCalledWith('/api/v1/sleep/sesiones/s-123/finalizar', {
      duracion: 10,
    });
  });
  ...
  it('listSleepSessions pasa limit como query param', async () => {
    ...
    expect(mockedGet).toHaveBeenCalledWith('/api/v1/sleep/sesiones', { params: { limit: 30 } });
  });

  it('listSleepDetections usa sessionId y limit', async () => {
    mockedGet.mockResolvedValueOnce({ data: [] });
    await listSleepDetections('s-9', 100);
    expect(mockedGet).toHaveBeenCalledWith('/api/v1/sleep/sesiones/s-9/detecciones', {
      params: { limit: 100 },
    });
  });
  ...
  it('submitSleepFeedback hace POST al feedback de la sesión', async () => {
    ...
    expect(mockedPost).toHaveBeenCalledWith('/api/v1/sleep/sesiones/s-1/feedback', {
      valoracion: 4,
    });
  });
});
...
describe('uploadSleepFragment', () => {
  ...
  it('construye FormData con el fragmento y envia multipart', async () => {
    ...
    expect(names).toEqual(expect.arrayContaining(['fragmento', 'fragment_index', 'duration_seconds', 'started_at']));
  });
...
describe('predictApnea', () => {
  ...
  it('hace POST multipart a /api/v1/sleep/v3/predict con params', async () => {
    ...
    expect(config.params).toEqual({ spo2: '95,94,93', modo: 'seguimiento', perfil: 'matias' });
    expect(config.headers['Content-Type']).toBe('multipart/form-data');
