...
const mockConstants = { appOwnership: null, executionEnvironment: 'standalone' };
...
jest.mock('react-native', () => ({
  ...
  Platform: {
    OS: 'ios',
    select: (specifics) => (specifics.ios !== undefined ? specifics.ios : specifics.default),
  },
}));
...
describe('triggerSevereApneaAlert', () => {
  ...
  it('no hace nada si está deshabilitado', async () => {
    const result = await triggerSevereApneaAlert({ ...BASE_SETTINGS, enabled: false }, { sessionId: 's1' });
  ...
  it('vibra y notifica si los métodos están activos', async () => {
    const settings = {
      ...BASE_SETTINGS,
      methods: { ...BASE_SETTINGS.methods, wake_alarm: true, notification: true },
    };
  ...
  it('no envía notificación si no se da permiso', async () => {
    ...
    await triggerSevereApneaAlert(
      { ...BASE_SETTINGS, methods: { ...BASE_SETTINGS.methods, notification: true } },
      { sessionId: 's1' },
    );
    expect(Notifications.scheduleNotificationAsync).not.toHaveBeenCalled();
  });
  ...
  it('no intenta contactos sin datos útiles', async () => {
    await triggerSevereApneaAlert(
      { ...BASE_SETTINGS, methods: { ...BASE_SETTINGS.methods, sms: true, whatsapp: true } },
      { sessionId: 's1' },
    );
